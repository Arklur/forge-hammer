...
let Kits = {
	...
	CreateUpgradeSchemes: ()=> {
		...
		if (FH.Main.BuildingUpgradePaths) { //NEW!!!
			...
			Object.entries(Kits.upgradeItems).forEach(([id, x]) => {
				Kits.Names[id] = x.upgradeItem.name;
			});
			...
			for (let startbuilding of Object.values(FH.Main.BuildingUpgradePaths)) {
				...
				for (let step of steps) {
					if (!schemes[step.fromBuildingId]) {
						...
						schemes[step.toBuildingId] = [...schemes[step.fromBuildingId], {buildingId: step.fromBuildingId, upgradeId: step.upgradeItemId}];	
					}
				}
				lower = Array.from(new Set(Object.values(schemes).flat().map(x=>x.buildingId)))
